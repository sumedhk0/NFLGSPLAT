...
def ankle_anchor(ga, gb, cam_a, cam_b, *, min_conf: float = 0.5, max_miss_m: float = 0.3):
    ...
    Measured on play 1 (2026-09-09, 2399 paired frames): the sideline's box-bottom point sits
    0.31 m from the triangulated ankles at the median (0.54 m pre-snap, the stances), and a
    two-view fit pinned there floats to satisfy both cameras and the pin at once."""
    from nfl_gsplat.calibration.endzone_paint import _closest_points, _rays
...
def two_view_pass(args, P, tracks, df, ground, blob):
    ...
    for pid in pids:
        ...
        for f in sorted(f for (f, p) in ka if p == pid):
            ...
            gnd.append(np.asarray(ground[f][pid], float) if anchor is None else anchor)
            n_anchor += anchor is not None
            init_bp.append(bp)
            init_go.append(go)
        if len(frames) < 8:
            continue
        ...
        jobs.append({"pid": pid, "frames": np.asarray(frames), "uv": uv, "conf": conf, "cams": cams,
                     ...
                     "cfg": {"min_conf": args.min_conf, "min_joints": args.min_joints, "max_iter": args.max_iter,
                             ...
                             "bounds_table": args.bounds_table, "hard_hinges": args.hard_hinges, "temporal_weight": args.temporal_weight,
...
def main() -> None:
    ...
    for pid, g in kdf.groupby("global_player_id"):
        ...
        jobs.append({"pid": pid, "frames": np.asarray(frames), "uv": np.stack(uv), "conf": np.stack(conf),
                     ...
                     "cfg": {"min_conf": args.min_conf, "min_joints": args.min_joints, "max_iter": args.max_iter,
                             "tilt_weight": args.tilt_weight, "tilt_free_deg": args.tilt_free_deg,
                             "bounds_weight": args.bounds_weight, "bounds_table": args.bounds_table, "hard_hinges": args.hard_hinges, "temporal_weight": args.temporal_weight,
