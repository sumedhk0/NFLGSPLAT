...
def refine_track(video_path, track, *, cfg=None, player_boxes_by_frame=None, log_every: int = 100):
    """Every frame of a CameraTrack refined to the paint; frames with too few
    segments keep their camera. Returns ``(CameraTrack, per-frame after_px)``."""
    import cv2
    ...
    for i, f in enumerate(frames):
        ...
        if res.applied:
            K[f], R[f], t[f] = res.K, res.R, res.t
            n_applied += 1
        after[f] = res.after_px
        if log_every and i % log_every == 0:
            _LOG.info("refine: frame %d  %.1f -> %.1f px over %d segments", f, res.before_px,
                      res.after_px, res.n_segments)
    cap.release()
    _LOG.info("refine: %d of %d frames refined; median after %.1f px", n_applied, len(frames),
              float(np.nanmedian(after)))
