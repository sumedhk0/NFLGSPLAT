...
CPU test suite: **157 passing** (calibration, tracking, field I/O, triangulation, pose fusion, ball Kalman + football asset, avatars, PLY merge, identity registry, avatar/shape library, scene compositing, per-stage CLI cores, end-to-end smoke). GitHub Actions runs `pytest -m "not gpu and not slow and not real_video"` + ruff on every push; see `.github/workflows/ci.yml`.
...
# Scaffold a play folder + meta.yaml, then drop in pre-trimmed clips
python scripts/new_play.py --season 2024 --week 1 --away NO --home ATL --play play_001
# → data/2024/week_01/NO_at_ATL/play_001/  (drop sideline.mp4 + endzone.mp4 here)

# Interactive: click landmarks on one frame per camera (per-play, manual pre-step)
python scripts/02_calibrate_cameras.py --play-dir data/2024/week_01/NO_at_ATL/play_001

bash scripts/04_process_play.sh --play-dir data/2024/week_01/NO_at_ATL/play_001
python scripts/05_render_novel_view.py --play-dir data/2024/week_01/NO_at_ATL/play_001 \
...
- **Roster prior coverage + alignment.** Player recognition is far more robust with the nflverse roster/participation prior, but participation data isn't available for every season, and home/away team mapping depends on the per-play `meta.yaml`. Without a roster the pipeline still runs in `identity.source=ocr_only` mode (OCR + jersey color), with no cross-game identity guarantees.
...
- `CalibrationError: reprojection error 11.3 px exceeds threshold 5.0 px` — re-annotate with more landmarks (`python scripts/02_calibrate_cameras.py --play-dir <play folder> --camera ...`), or pick a different keyframe. See SETUP.md §3.
